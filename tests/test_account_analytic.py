...
class TestAccountAnalyticAccount(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        cls.env.user.write({
            'groups_id': [
                (4, cls.env.ref('account_analytic_report.group_analytic_accounting').id),
                (4, cls.env.ref('account_analytic_report.group_analytic_accounting').id),
            ],
        })
        ...
        cls.test_analytic_account = cls.env['account.account'].create({'name': 'test_analytic_account'})
        ...
        self.env['account.move'].create({
            ...
            'line_ids': [
                (0, 0, {
                    ...
                    'branch_analytic_tag_ids': [(6, 0, self.test_analytic_tag.ids)],
                }),
                (0, 0, {
                    'name': 'line_credit',
                    'account_id': self.company_data['default_account_revenue'].id,
